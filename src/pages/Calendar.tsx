        ...
        <div className="max-w-[1600px] mx-auto px-3 sm:px-6 lg:px-8">
          ...
          <div className="flex items-center justify-between gap-3 mb-4 flex-wrap">
            <div className="flex items-center gap-2.5">
              <CalendarIcon className="w-5 h-5 text-foreground" aria-hidden="true" />
              <h1 className="text-xl sm:text-2xl font-bold text-foreground tracking-tight">Calendrier</h1>
              {totalCount > 0 && (
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="flex items-center border border-border">
                ...
                <Button
                  ...
                  className="h-8 px-3 text-xs uppercase tracking-wider rounded-none border-r border-border"
                >
                  Aujourd'hui
                </Button>
                <Button
          ...
          {/* Week view */}
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-7 gap-0 border border-border bg-background">
            {days.map((day, idx) => {
                ...
                >
                  ...
                  <div className={cn(
                    ...
                    <div>
                      ...
                      <p className={cn(
                        'text-lg font-bold font-mono leading-none mt-0.5',
          ...
          {!isLoading && totalCount === 0 && (
            <div className="mt-6 text-center py-12 border border-dashed border-border">
              <CalendarIcon className="w-8 h-8 text-muted-foreground/40 mx-auto mb-3" aria-hidden="true" />
              <h3 className="text-sm font-bold uppercase tracking-wider mb-1">Pas d'événement cette semaine</h3>
              <p className="text-xs text-muted-foreground max-w-md mx-auto">
                Les entretiens (qualifications), InMails programmés et étapes de séquence à venir s'afficheront ici.
              </p>
            </div>
          )}
        </div>
...
const EventCard = React.memo(function EventCard({ event }: { event: CalendarEvent }) {
  ...
  return (
    <div
      className={cn(
        'border p-2 hover:shadow-sm transition-shadow cursor-pointer',
        style.color,
        isPast && 'opacity-60',
      )}
      title={event.title}
