...
export const AgentMessageBubble: React.FC<AgentMessageBubbleProps> = ({ message, isStreaming, onQuickReply, isLastAssistant }) => {
  const isUser = message.role === 'user';
  const isStatus = message.role === 'status';

  // Extract options from content
  const optionsMatch = message.content.match(/\[OPTIONS\]\s*(\[[\s\S]*?\])\s*\[\/OPTIONS\]/);
  let options: string[] = [];
  if (optionsMatch) {
  ...
  return (
    <div className={cn("flex flex-col", isUser ? "items-end" : "items-start")}>
      ...
      {!isUser && !isStreaming && options.length > 0 && isLastAssistant && onQuickReply && (
        <div className="flex flex-wrap gap-1.5 mt-2 ml-5">
          {options.map((opt, i) => (
            <button
              key={i}
              onClick={() => onQuickReply(opt)}
              className="px-2.5 py-1.5 text-[10px] font-medium border border-foreground bg-background text-foreground hover:bg-foreground hover:text-background transition-colors active:scale-95"
            >
              {opt}
            </button>
          ))}
