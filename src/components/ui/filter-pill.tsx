 ...
 * d'accent (la sélection est un usage de l'accent, 01-direction.md, § 2).
 *
 * Usage :
    ...
    <Popover>
      <PopoverTrigger asChild>
        <button
          ...
          className={cn(
            "inline-flex h-8 shrink-0 items-center gap-1.5 whitespace-nowrap rounded-lg border px-3 text-sm font-medium text-foreground transition-colors duration-150 hover:bg-accent focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring data-[state=open]:bg-accent [&_svg]:size-3.5 [&_svg]:shrink-0",
...
export function FilterOption({ checked, onCheckedChange, children, description }: FilterOptionProps) {
  return (
    <label className="flex cursor-pointer items-start gap-2.5 rounded-md px-2 py-1.5 text-sm text-foreground transition-colors hover:bg-accent">
