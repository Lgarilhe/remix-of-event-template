...
import { Plus, Trash2, Pencil, Settings as SettingsIcon, Building2, GraduationCap, Briefcase, Shield, AlertCircle } from 'lucide-react';
...
const PresetFormDialog: React.FC<{
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    try {
      // Cleanup : retirer les champs vides du requirements
      const cleaned: PedigreeRequirements = {
        ...(requirements.schools_required?.length ? { schools_required: requirements.schools_required } : {}),
        ...(requirements.diploma_must_be_from && requirements.diploma_must_be_from !== 'any' ? { diploma_must_be_from: requirements.diploma_must_be_from } : {}),
        ...(requirements.companies_required_provenance?.length ? { companies_required_provenance: requirements.companies_required_provenance } : {}),
        ...(requirements.companies_specific_required?.length ? { companies_specific_required: requirements.companies_specific_required } : {}),
        ...(requirements.companies_avoid?.length ? { companies_avoid: requirements.companies_avoid } : {}),
        ...(requirements.min_seniority ? { min_seniority: requirements.min_seniority } : {}),
        ...(requirements.strict_mode ? { strict_mode: true } : {}),
        ...(requirements.custom_instructions?.trim() ? { custom_instructions: requirements.custom_instructions.trim().slice(0, 400) } : {}),
      };
      await onSubmit({
        name: name.trim(),
        ...
        <div className="space-y-5 py-2">
          ...
          <div className="space-y-3 pl-1">
            <div>
              ...
              <CsvInput
                value={requirements.schools_required || []}
                onChange={(v) => setRequirements({ ...requirements, schools_required: v })}
            ...
            <div>
              ...
              <Select
                value={requirements.diploma_must_be_from || 'any'}
                onValueChange={(v) => setRequirements({ ...requirements, diploma_must_be_from: v as PedigreeRequirements['diploma_must_be_from'] })}
              >
          ...
          <div className="space-y-3 pl-1">
            <div>
              ...
              <MultiSelectPills
                ...
                value={requirements.companies_required_provenance || []}
                onChange={(v) => setRequirements({ ...requirements, companies_required_provenance: v as CompanyProvenance[] })}
              />
            </div>
            <div>
              <Label>Entreprises spécifiques requises (au moins une suffit)</Label>
              <CsvInput
                value={requirements.companies_specific_required || []}
                onChange={(v) => setRequirements({ ...requirements, companies_specific_required: v })}
            ...
            <div>
              ...
              <MultiSelectPills
                ...
                value={requirements.companies_avoid || []}
                onChange={(v) => setRequirements({ ...requirements, companies_avoid: v as CompanyAvoidCategory[] })}
              />
            </div>
          </div>
          ...
          <div className="space-y-3 pl-1">
            <div>
              ...
              <Select
                value={requirements.min_seniority || 'none'}
                onValueChange={(v) => setRequirements({ ...requirements, min_seniority: v === 'none' ? undefined : v as PedigreeRequirements['min_seniority'] })}
              >
            ...
            <div className="flex items-start justify-between p-3 border border-border rounded-md gap-3">
              ...
              <Switch
                id="strict-mode"
                checked={requirements.strict_mode || false}
                onCheckedChange={(v) => setRequirements({ ...requirements, strict_mode: v })}
              />
            </div>
          </div>
          ...
          <div>
            ...
            <Textarea
              ...
              value={requirements.custom_instructions || ''}
              onChange={(e) => setRequirements({ ...requirements, custom_instructions: e.target.value })}
              rows={3}
              maxLength={400}
            />
            <p className="text-xs text-muted-foreground mt-1">
              {(requirements.custom_instructions?.length || 0)} / 400 caractères
            </p>
          </div>
        </div>
