  ...
  const handleConnect = async (provider: 'GOOGLE' | 'OUTLOOK' | 'IMAP') => {
    ...
    try {
      ...
      if (data?.success && (data as any).url) {
        ...
        toast.info(`Une fenêtre de connexion ${providerName} s'est ouverte. Cliquez sur "Rafraîchir" une fois la connexion effectuée.`);
      } else {
      ...
      <CardContent className="space-y-4">
        {myMapping && myAccount ? (
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm font-medium text-foreground">
                  {myMapping.email_address || myAccount.name || myAccount.identifier}
                </p>
          ...
          <div className="p-3 bg-muted/50 rounded-lg text-sm text-muted-foreground">
            Compte lié : {myMapping.email_address || myMapping.email_account_id}
