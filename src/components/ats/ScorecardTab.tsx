...
import { Loader2, Sparkles, Star, Save, RotateCcw, ChevronDown, ChevronUp, Pencil, Check, Plus, Trash2 } from 'lucide-react';
...
export const ScorecardTab: React.FC<ScorecardTabProps> = ({ candidate, enrichedProfile }) => {
  ...
  const handleGenerate = useCallback(async () => {
    ...
    try {
      ...
      const { data, error } = await supabase.functions.invoke('generate-scorecard', {
        body: { candidateProfile, jobContext, scoringDetails: candidate.scoringDetails },
      });
  ...
  }, [candidate, enrichedProfile, activeIndex]);
