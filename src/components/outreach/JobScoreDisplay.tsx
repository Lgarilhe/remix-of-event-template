  ...
  return (
    <Tooltip>
      <TooltipTrigger asChild>
        <span className="inline-flex items-center gap-1 px-1.5 py-0.5 text-xs font-medium border border-border text-muted-foreground cursor-help rounded-none bg-background">
...
export const JobScoreDisplay: React.FC<JobScoreDisplayProps> = ({ result, jobTitle, compact = false }) => {
  if (result.error) {
    return (
      <div className="bg-muted border border-border p-3 text-sm text-foreground rounded-none">
        Erreur: {result.error}
      </div>
    );
  }
  ...
  if (result.hardFilterPassed === false && result.hardFilterKO) {
    return (
      <div className="flex items-start gap-2.5 p-3 border border-border bg-muted rounded-none">
  ...
  const confidenceBadge = result.confidenceScore != null && result.confidenceScore < 70 ? (
    <Tooltip>
      <TooltipTrigger asChild>
        <span className="inline-flex items-center gap-1 px-1.5 py-0.5 text-xs font-medium border border-border text-muted-foreground cursor-help rounded-none bg-muted">
  ...
  if (compact) {
    return (
      <div className="flex items-center gap-3 p-2 border border-border bg-muted/20 rounded-none">
