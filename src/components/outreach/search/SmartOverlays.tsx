...
async function fetchFundedCompanies(stage: string, scope: FundedScope): Promise<FundedStage> {
  ...
  let query = supabase
    ...
    .select('canonical_name, linkedin_company_id, headcount_growth_6m')
    .eq('funding_stage', stage)
    .not('linkedin_company_id', 'is', null);
  if (scope === 'FR') query = query.eq('country', 'FR');
  // « Top » = croissance d'effectifs 6 mois décroissante : une boîte qui
  // grossit recrute MAINTENANT. (Date de levée et effectif absolu ne sont
  // pas fournis par la recherche du fournisseur — enrichissement payant.)
  const { data, error } = await query
    .order('headcount_growth_6m', { ascending: false, nullsFirst: false })
