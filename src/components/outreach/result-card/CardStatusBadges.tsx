...
export const CardStatusBadges: React.FC<CardStatusBadgesProps> = ({
  ...
  return (
    <>
      {candidateStatus && (
        <>
          {candidateStatus.status === 'messaged' && (
            <Badge className="bg-blue-500 text-white text-[10px] px-1.5 py-0 h-4 sm:h-5 gap-0.5 shrink-0 rounded-none border border-blue-600">
              <MessageSquare className="w-2.5 h-2.5" />
              <span className="hidden sm:inline">Contacté</span>
            </Badge>
          )}
          {candidateStatus.status === 'replied' && (
            <Badge className="bg-emerald-500 text-white text-[10px] px-1.5 py-0 h-4 sm:h-5 gap-0.5 shrink-0 rounded-none border border-emerald-600">
              <CheckCircle2 className="w-2.5 h-2.5" />
              <span className="hidden sm:inline">Répondu</span>
            </Badge>
          )}
          {candidateStatus.status === 'shortlisted' && (
            <Badge className="bg-amber-500 text-white text-[10px] px-1.5 py-0 h-4 sm:h-5 gap-0.5 shrink-0 rounded-none border border-amber-600">
              <Star className="w-2.5 h-2.5" />
              <span className="hidden sm:inline">Shortlist</span>
            </Badge>
          )}
          {candidateStatus.status === 'scored' && !jobScore && (
            <Badge variant="outline" className="text-[10px] px-1.5 py-0 h-4 sm:h-5 gap-0.5 shrink-0 text-purple-600 border-purple-500 bg-purple-50 rounded-none">
              <Target className="w-2.5 h-2.5" />
              {candidateStatus.score && <span>{candidateStatus.score}%</span>}
            </Badge>
          )}
          {candidateStatus.status === 'dismissed' && (
            <Badge variant="outline" className="text-[10px] px-1.5 py-0 h-4 sm:h-5 gap-0.5 shrink-0 text-orange-500 border-orange-500 bg-orange-50 rounded-none">
      ...
      {profile.premium && (
        <Badge variant="outline" className="text-[10px] px-1 sm:px-1.5 py-0 h-4 sm:h-5 text-amber-600 border-amber-500 bg-amber-50 shrink-0 rounded-none">
          <Star className="w-2.5 h-2.5 sm:w-3 sm:h-3 mr-0.5 fill-amber-400" />
          <span className="hidden sm:inline">Premium</span>
        </Badge>
      )}
      {profile.open_to_work && (
        <Badge className="bg-green-500 text-white text-[10px] px-1 sm:px-1.5 py-0 h-4 sm:h-5 gap-0.5 shrink-0 rounded-none border border-green-600">
      ...
      {isLikelyToRespond && (
        <Badge variant="outline" className="text-[10px] px-1 sm:px-1.5 py-0 h-4 sm:h-5 text-purple-600 border-purple-500 bg-purple-50 shrink-0 hidden sm:flex rounded-none">
          <Sparkles className="w-3 h-3 mr-0.5" />
          Réactif
        </Badge>
      )}
      {airtableMatch && (
        <Badge className={`text-[10px] px-1.5 py-0 h-4 sm:h-5 shrink-0 gap-1 cursor-default rounded-none ${
          airtableMatch.match_type === 'fuzzy'
            ? 'bg-teal-100 text-teal-700 border border-dashed border-teal-500'
      ...
      {notionMatch && (
        <Tooltip>
          <TooltipTrigger asChild>
            <Badge variant="outline" className="text-[10px] px-1 py-0 h-4 sm:h-5 border-foreground/30 bg-muted shrink-0 rounded-none">
