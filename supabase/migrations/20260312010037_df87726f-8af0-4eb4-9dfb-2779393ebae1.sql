...
CREATE FUNCTION get_vivier_contacts(
  p_limit int,
  p_offset int,
  p_source_base text DEFAULT NULL,
  p_contact_type text DEFAULT NULL,
  p_min_shortlists int DEFAULT 1,
...
AS $$
  ...
  contact_stats AS (
    ...
    WHERE GREATEST(COALESCE(sc.cnt, 0), COALESCE(sco.cnt, 0)) >= p_min_shortlists
      ...
      AND (p_search IS NULL OR unaccent(ct.full_name) ILIKE '%' || unaccent(p_search) || '%' OR ct.email ILIKE '%' || p_search || '%' OR unaccent(co.name) ILIKE '%' || unaccent(p_search) || '%')
  )
  SELECT
    cs.airtable_id,
    cs.full_name,
