...
-- Fix get_vivier_contacts: also link contacts to companies via shortlists
...
AS $$
  ...
  -- Resolve company from shortlist when contact has no company_airtable_id
  contact_company_via_shortlist AS (
    SELECT DISTINCT ON (s.contact_airtable_id)
      s.contact_airtable_id AS cid,
      s.company_airtable_id AS coid
...
-- Fix get_vivier_companies: also count contacts linked via shortlists
...
AS $$
  ...
  -- Count contacts linked directly OR via shortlists
  company_contacts AS (
