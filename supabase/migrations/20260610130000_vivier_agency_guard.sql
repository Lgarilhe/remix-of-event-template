-- ====================================================================
-- SECURITY: garde serveur "agency-only" sur les fonctions vivier
--
-- Contexte (audit 2026-06-10, addendum) : get_vivier_contacts et
-- get_vivier_companies sont SECURITY DEFINER (bypass RLS sur les tables
-- airtable_*) et ne vérifiaient PAS l'org de l'appelant. Le gate
-- "Prospection = agency-only" n'existait que côté client (featureGates.ts)
-- → tout utilisateur authentifié d'une org enterprise/freelance pouvait
-- lire le CRM vivier via POST /rest/v1/rpc/get_vivier_contacts.
--
-- Fix : redéfinition des deux fonctions (corps identique à la dernière
-- version, migration 20260409140625) avec un garde en tête :
--   - service_role passe (les agent-tools appellent ces fonctions via
--     adminClient, auth.uid() est NULL dans ce cas)
--   - sinon l'appelant doit être membre d'une org org_type = 'agency'
--   - sinon erreur 42501 (message générique, pas de détail interne)
--
-- CREATE OR REPLACE conserve les grants existants (authenticated +
-- service_role, anon révoqué par 20260610120000).
-- get_vivier_candidates n'est pas gardée : son EXECUTE client a été
-- entièrement révoqué (appelée uniquement via service_role).
-- ====================================================================
...
AS $$
BEGIN
  -- Garde agency-only : service_role passe (agent-tools via adminClient),
  -- sinon membre d'une org de type 'agency' requis.
