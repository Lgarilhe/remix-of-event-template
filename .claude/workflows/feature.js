export const meta = {
  ...
  phases: [
    { title: 'Cadrage', detail: 'archi découpe la tâche (front / edge / data / risques / hors-scope)' },
    { title: 'Implémentation', detail: 'frontend-dev + edge-dev en parallèle sur leurs zones disjointes' },
    { title: 'Revue', detail: 'security-reviewer (adversaire) + archi (conformité) sur le diff' },
  ],
}
...
const plan = await agent(
  ...
  { agentType: 'archi', phase: 'Cadrage', label: 'archi:cadrage', schema: PLAN }
)
...
if (plan.frontendTasks.length)
  jobs.push(() => agent(
    ...
    { agentType: 'frontend-dev', phase: 'Implémentation', label: 'frontend-dev' }
  ))
if (plan.edgeTasks.length)
  jobs.push(() => agent(
    `Implémente la part EDGE / BACKEND.\n\nContexte : ${plan.summary}\n\nTâches :\n- ${plan.edgeTasks.join('\n- ')}\n\n` +
    `Data/migrations à prévoir : ${(plan.dataTasks || []).join(' ; ') || '—'}\nHORS scope : ${oos}`,
    { agentType: 'edge-dev', phase: 'Implémentation', label: 'edge-dev' }
  ))
...
const [security, conformity] = await parallel([
  () => agent(
    ...
    { agentType: 'security-reviewer', phase: 'Revue', label: 'security-review' }
  ),
  () => agent(
    ...
    { agentType: 'archi', phase: 'Revue', label: 'archi:conformité' }
  ),
])
