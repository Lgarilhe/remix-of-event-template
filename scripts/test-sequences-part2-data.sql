-- ============================================================================
-- TEST COMPLET PARTIE 2 : Webhooks, Tracking, Wait Events, Timeouts,
-- Quotas, Company Stop, Business Hours, Deep Branching, Duplicate Prevention,
...
BEGIN
  ...
  -- ================================================================
  -- T2-10: WEBHOOK — LinkedIn reply (message_received)
  -- Simule l'état AVANT le webhook, puis le webhook sera appelé en HTTP
  -- ================================================================
  ...
  -- Candidat 1 : a reçu un message (status sent) — le webhook devrait le passer en replied
  ...
  -- Step 2 schedulé (devrait être cancelled après reply)
  ...
  -- Candidat 2 : même company (ACME Corp) — devrait être stoppé par company_reply
  INSERT INTO sequence_enrollments (sequence_id, account_id, profile_id, profile_name, company_name, status, organization_id, created_by)
  ...
  -- ================================================================
  -- T2-11: WEBHOOK — Invite accepted (new_relation)
  -- ================================================================
  ...
  RETURNING id INTO v_enroll;
  -- Step 1 déjà sent, step 2 en waiting_event
  INSERT INTO sequence_step_executions (enrollment_id, step_id, step_order, scheduled_at, status, executed_at, organization_id)
  ...
  -- ================================================================
  -- T2-12: EMAIL TRACKING — pixel open + click
  -- ================================================================
  INSERT INTO outreach_sequences (name, organization_id, is_active, created_by)
  ...
  RAISE NOTICE 'T2-12 Tracking ID for HTTP tests: %', v_tracking_id;
  RAISE NOTICE 'Open URL: /functions/v1/sequence-email-track?tid=%&evt=open', v_tracking_id;
  RAISE NOTICE 'Click URL: /functions/v1/sequence-email-track?tid=%&evt=click&url=https%%3A%%2F%%2Fgoogle.com', v_tracking_id;
  ...
  -- Step 1: wait_connection avec timeout 2 jours
  ...
  -- Simuler un waiting_event créé il y a 3 jours (timeout = 2 → devrait brancher)
  ...
  -- Insérer 2 executions scheduled pour le même enrollment+step (ne devrait pas arriver en prod)
  INSERT INTO sequence_step_executions (enrollment_id, step_id, step_order, scheduled_at, status, organization_id)
  ...
  -- ================================================================
  -- T2-15: DEEP BRANCHING (branche dans une branche — 3 niveaux)
  -- ================================================================
  INSERT INTO outreach_sequences (name, organization_id, is_active, created_by)
  VALUES ('[T2-15] Deep branching', v_org_id, true, v_user_id)
  RETURNING id INTO v_seq;

  -- Level 0: condition if_has_email
  ...
  -- Level 1 YES: condition if_has_phone (nested)
  ...
  -- Level 2 YES-NO: WhatsApp fallback
  ...
  -- Candidat avec email + phone (devrait aller: cond→yes→cond→yes→email)
  ...
  -- ================================================================
  -- T2-16: BUSINESS HOURS (sans force=true)
  -- Schedule un step à 3h du matin → devrait être reschedulé
  -- ================================================================
  INSERT INTO outreach_sequences (name, organization_id, is_active, created_by)
  ...
  RETURNING id INTO v_enroll;
  -- NOTE: ce test doit être lancé SANS force=true pour vérifier le respect des business hours
  INSERT INTO sequence_step_executions (enrollment_id, step_id, step_order, scheduled_at, status, organization_id)
  VALUES (v_enroll, v_step, 1, now(), 'scheduled', v_org_id);

  -- ================================================================
  -- T2-17: ENROLLMENT LIFECYCLE (pause → resume)
  -- ================================================================
  INSERT INTO outreach_sequences (name, organization_id, is_active, created_by)
  ...
  -- Candidat replied → execution doit être skippée
  ...
  -- Candidat stopped → execution doit être skippée
  INSERT INTO sequence_enrollments (sequence_id, account_id, profile_id, profile_name, email_used, status, organization_id, created_by)
  ...
  -- Candidat completed → execution doit être skippée
  ...
  -- ================================================================
  -- T2-18: CONDITION if_email_opened (check tracking_data)
  -- ================================================================
  INSERT INTO outreach_sequences (name, organization_id, is_active, created_by)
  ...
  -- Le candidat est désinscrit (suppressed_emails déjà inséré dans part 1)
  INSERT INTO sequence_enrollments (sequence_id, account_id, profile_id, profile_name, email_used, status, organization_id, created_by)
  ...
  RAISE NOTICE '';
  RAISE NOTICE 'ÉTAPES :';
  RAISE NOTICE '1. Appeler process-sequences avec force=true (pour la plupart des tests)';
  RAISE NOTICE '2. Appeler check_timeouts pour T2-13';
  RAISE NOTICE '3. Appeler les webhooks HTTP pour T2-10 et T2-11';
  RAISE NOTICE '4. Appeler le tracking pixel pour T2-12';
  RAISE NOTICE '5. Appeler process-sequences SANS force pour T2-16 (business hours)';
  RAISE NOTICE '============================================';
